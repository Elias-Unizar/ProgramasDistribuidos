...
* ASIGNATURAS: 30221/39521 - Sistemas Distribuidos
...
// sendRequest envía una petición (id, interval) al servidor. Una petición es
// un par id (el identificador único de la petición) e interval, el intervalo
// en el cual se desea que el servidor encuentre los números primos. La petición
// se serializa utilizando el encoder y una vez enviada la petición se almacena
// en una estructura de datos, junto con una estampilla temporal. Para evitar
// condiciones de carrera, la estructura de datos compartida se almacena en una
// Goroutine (handleRequests) y que controla los accesos a través de canales
// síncronos. En este caso, se añade una nueva petición a la estructura de datos
// mediante el canal requestTimeChan
func sendRequest(
	endpoint string,
	id int,
...
// Por el canal replayTimeChan se indica que ha llegado una respuesta de una
// petición. En la respuesta, se obtiene también el timestamp de la recepción.
...
func handleRequestsDelays(requestTimeChan chan com.TimeCommEvent,
	replayTimeChan chan com.TimeCommEvent) {
	...
	for {
		...
		case timeReplay := <-replayTimeChan:
			log.SetFlags(log.Lshortfile | log.Lmicroseconds)
			delay += timeReplay.T.Sub(requestsTimes[timeReplay.Id])
			log.Println("-> Delay : ",
				timeReplay.T.Sub(requestsTimes[timeReplay.Id]),
				", between request ", timeReplay.Id,
				" and its reply")
			log.Println("Tiempo medio de respuesta hasta ahora: ", delay/time.Duration(timeReplay.Id))
...
func receiveReply(conn net.Conn, replayTimeChan chan com.TimeCommEvent) {
	...
	// log.SetFlags(log.Lshortfile | log.Lmicroseconds)
	// log.Println("Client receive reply for request Id with Primes output=\n",
	// 	reply)
...
func main() {
	...
	for i := 0; i < numIt; i++ {
		for t := 1; t <= requestTmp; t++ {
			sendRequest(endpoint, i*requestTmp+t, interval,
				requestTimeChan, replayTimeChan)
		}
